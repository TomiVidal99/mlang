...
import { BasicType } from "./grammar";
import { OctaveMatlabSyntaxListener } from "./OctaveMatlabSyntaxListener";
import { Assignment_statementContext, ProgramContext, VariableContext } from "./OctaveMatlabSyntaxParser";
...
export class TreeListener implements OctaveMatlabSyntaxListener {
  enterProgram(ctx: ProgramContext): void {
    // log(ctx.toStringTree());
  ...
  enterVariable(ctx: VariableContext): void {
    log("Entering variable: " + ctx.ID().text);
  }

  enterAssignment_statement(ctx: Assignment_statementContext): void {
    const assigmentStatement = ctx.text;
    log("Entering variable");
  }
